  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (profile?.role === 'admin') {
        ...
        const url = plan ? `${redirectTo}?plan=${plan}` : redirectTo;
        router.push(url);
      }
    } catch (err: any) {
    ...
    <div className="w-full max-w-md bg-[#050505] border border-white/10 p-10 rounded-[2.5rem] shadow-2xl relative z-10 backdrop-blur-xl">
      ...
      <div className="mt-8 pt-8 border-t border-white/5 text-center space-y-4">
        <p className="font-body text-sm text-white/40">
          Scanning for access? <Link href={`/register${plan ? `?plan=${plan}` : ''}`} className="text-tcg-green hover:underline">Register New Node</Link>
        </p>
